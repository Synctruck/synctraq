...
function Unassigned() {
    ...
    const listAllUnassigned = (pageNumber, dataView) => {
        ...
        fetch(url_general +'unassigned/list/'+ dataView +'/'+ idTeam +'/?page='+ pageNumber)
        .then(res => res.json())
        .then((response) => {
    ...
    const handlerValidation = (e) => {
        ...
        fetch(url_general +'unassigned/insert', {
            headers: { "X-CSRF-TOKEN": token },
            method: 'post',
            body: formData
        })
        ...
        return (
            ...
            <tr key={i} className="alert-danger">
                ...
                <td style={ { width: '100px'} }>
                    { assigned.unassignedDate ? assigned.unassignedDate.substring(5, 7) +'-'+ assigned.unassignedDate.substring(8, 10) +'-'+ assigned.unassignedDate.substring(0, 4) : '' }
                </td>
                <td>
...
if (document.getElementById('unassigned')) {
    ReactDOM.render(<Unassigned />, document.getElementById('unassigned'));
}