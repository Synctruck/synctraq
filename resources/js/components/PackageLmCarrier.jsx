...
function PackageLmCarrier() {
    ...
    const listAllPackageInbound = (pageNumber, route, state) => {
        ...
        fetch(url_general +'package-lm-carrier/list/'+ idCompany +'/'+ dateStart+'/'+ dateEnd +'/'+ route +'/'+ state +'/?page='+ pageNumber)
        .then(res => res.json())
        .then((response) => {
    ...
    const handlerImport = (e) => {
        ...
        fetch(url_general +'package-lm-carrier/import', {
            headers: { "X-CSRF-TOKEN": token },
            method: 'post',
            body: formData
        })
...
export default PackageLmCarrier;

// DOM element
if (document.getElementById('packageLmCarrier')) {
    ReactDOM.render(<PackageLmCarrier />, document.getElementById('packageLmCarrier'));
}