...
function ReportLmCarrier() {
    ...
    useEffect(() => {
        ...
        listReportLmCarrier(page, RouteSearch, StateSearch);

    }, [ idCompany, dateInit, dateEnd ]);


    const listReportLmCarrier = (pageNumber, routeSearch, stateSearch) => {

        setIsLoading(true);

        fetch(url_general +'report/list/lm-carrier/'+ idCompany +'/'+ dateInit +'/'+ dateEnd +'/'+ routeSearch +'/'+ stateSearch +'?page='+ pageNumber)
        .then(res => res.json())
        .then((response) => {
    ...
    const handlerChangePage = (pageNumber) => {
        ...
        listReportLmCarrier(pageNumber, RouteSearch, StateSearch);
    }
    ...
    const handlerExport = (type) => {
        ...
        {
            let url = url_general +'report/export/lm-carrier/'+ idCompany +'/'+ dateInit +'/'+ dateEnd +'/'+ RouteSearch +'/'+ StateSearch +'/'+ type;
    ...
    const handlerChangeRoute = (routes) => {
        ...
        if(routes.length != 0)
        {
            ...
            listReportLmCarrier(1, routesSearch, StateSearch);
        }
        else
        {
            setRouteSearch('all');

            listReportLmCarrier(1, 'all', StateSearch);
        }
    };
    ...
    const handlerChangeState = (states) => {
        ...
        if(states.length != 0)
        {
            ...
            listReportLmCarrier(1, RouteSearch, statesSearch);
        }
        else
        {
            setStateSearch('all');

            listReportLmCarrier(1, RouteSearch, 'all');
        }
    };
                                    ...
                                    <div className="col-lg-2 form-group" style={ {paddingLeft: (isLoading ? '5%' : '')} }>
                                        {
                                            (
                                                ...
                                                :
                                                    <b className="alert-success" style={ {borderRadius: '10px', padding: '10px'} }>LM CARRIER: { quantityMMS }</b>
                                            )
                                        }
                                    </div>
...
export default ReportLmCarrier;

if (document.getElementById('reportLmCarrier'))
{
    ReactDOM.render(<ReportLmCarrier />, document.getElementById('reportLmCarrier'));
}
