...
function PackageFailed() {
    ...
    useEffect(() => {
        ...
        listFailed(1, RouteSearch, StateSearch);

    }, [dateInit, dateEnd, idTeam, idDriver,idCompany]);


    const listFailed = async (pageNumber, routeSearch, stateSearch) => {
        ...
        .then((response) => {
            ...
            setTotalPackage(response.packageFailedList.total);
            setTotalPage(response.packageFailedList.per_page);
            setPage(response.packageFailedList.current_page);
            setQuantityDispatch(response.packageFailedList.total);
    ...
    const handlerChangePage = (pageNumber) => {
        ...
        listFailed(pageNumber, RouteSearch, StateSearch);
    }
    ...
    const handlerChangeRoute = (routes) => {
        ...
        if(routes.length != 0)
        {
            ...
            listFailed(1, routesSearch, StateSearch);
        }
        else
        {
            setRouteSearch('all');

            listFailed(1, 'all', StateSearch);
        }
    };
    ...
    const handlerChangeState = (states) => {
        ...
        if(states.length != 0)
        {
            ...
            listFailed(page, RouteSearch, statesSearch);
        }
        else
        {
            setStateSearch('all');

            listFailed(page, RouteSearch, 'all');
        }
    };
    ...
    const handlerRedirectDispatch = () => {

        location.href = url_general +'package-dispatch';
    }

    const handlerRedirectReInbound = () => {

        location.href = url_general +'package/return';
    }

    const handlerRedirectFailedReport = () => {

        location.href = url_general +'report/failed';
    }
                                        ...
                                        <div className="row form-group">
                                            <div className="col-lg-2">
                                                <div className="form-group">
                                                    <button className="btn btn-success btn-sm form-control" onClick={ handlerRedirectDispatch }>DISPATCH</button>
                                                </div>
                                            </div>
                                            <div className="col-lg-2">
                                                <div className="form-group">
                                                    <button className="btn btn-danger btn-sm form-control" onClick={ handlerRedirectReInbound }>RE INBOUND</button>
                                                </div>
                                            </div>
                                            <div className="col-lg-2">
                                                <div className="form-group">
                                                    <button className="btn btn-warning btn-sm form-control" onClick={ handlerRedirectFailedReport }>FAILED REPORT</button>
                                                </div>
                                            </div>
                                        </div>
...
if (document.getElementById('packageFailed'))
{
    ReactDOM.render(<PackageFailed />, document.getElementById('packageFailed'));
}
