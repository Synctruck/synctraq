...
function PackageMiddleMileScan() {
    ...
    const listAllPackageWarehouse = (pageNumber, route, state) => {
        ...
        fetch(url_general +'package-mms/list/'+ idCompany +'/'+ idValidator +'/'+ dateStart+'/'+ dateEnd +'/'+ route +'/'+ state +'/?page='+ pageNumber)
        .then(res => res.json())
        .then((response) => {
    ...
    const handlerOpenModal = (PACKAGE_ID) => {
        ...
        fetch(url_general +'package-mms/get/'+ PACKAGE_ID)
        .then(res => res.json())
        .then((response) => {
    ...
    const handlerInsert = (e) => {
        ...
        if(sendWarehouse)
        {
            ...
            fetch(url_general +'package-mms/insert', {
                headers: { "X-CSRF-TOKEN": token },
                method: 'post',
                body: formData
            })
            ...
            .then((response) => {
                    ...
                    {
                        setTextMessage("NO INBOUND #"+ Reference_Number_1);
                        setTypeMessage('error');
                        setNumberPackage('');
                    ...
                    else if(response.stateAction == 'packageInWarehouse')
                    {
                        ...
                        setTextMessage("The package is in MIDDLE MILE SCAN STATUS #"+ Reference_Number_1);
                        setTextMessageDate('');
                        setTypeMessage('warning');
                        setNumberPackage('');
    ...
    const handlerImport = (e) => {
        ...
        fetch(url_general +'package-mms/import', {
            headers: { "X-CSRF-TOKEN": token },
            method: 'post',
            body: formData
        })
    ...
    const handlerViewPDF = (Reference_Number) => {
        ...
        window.open(url_general +'package-mms/pdf-label/'+ Reference_Number);
    }
                ...
                <td>
                    {
                        pack.user
                        ?
                            <b>{ pack.user.name +' '+ pack.user.nameOfOwner }</b>
                        :
                            ''
                    }
                </td>
...
export default PackageMiddleMileScan;

// DOM element
if (document.getElementById('packageMiddleMileScan')) {
    ReactDOM.render(<PackageMiddleMileScan />, document.getElementById('packageMiddleMileScan'));
}
