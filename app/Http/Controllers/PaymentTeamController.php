    ...
    public function SendReceipt($idPayment)
{
    ...
    // ... (Código adicional para agregar más datos al archivo CSV)

    $filePath = public_path('receipts/' . $filename);
    file_put_contents($filePath, stream_get_contents($file));
    fclose($file);

    $user = Auth::user();
    $email = $user->email;

    Mail::send([], [], function($message) use ($email, $filePath, $filename) {
        $message->to($email)
