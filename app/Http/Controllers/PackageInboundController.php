    ...
    private function getDataInbound($idCompany, $dateStart,$dateEnd, $route, $state,$type='list'){
        ...
        if($type =='list')
        {
            $packageListInbound = $packageListInbound->where('reInbound', 0)
                                                ...
                                                ->select('company', 'TRUCK', 'Reference_Number_1', 'Dropoff_Contact_Name', 'Dropoff_Contact_Phone_Number', 'Dropoff_Address_Line_1', 'Dropoff_City', 'Dropoff_Province', 'Dropoff_Postal_Code', 'Weight', 'Route', 'idCellar','nameCellar','cityCellar','stateCellar','created_at')
                                                ->paginate(50);
        }
        else{
